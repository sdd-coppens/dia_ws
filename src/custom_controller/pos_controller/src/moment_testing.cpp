...
private:
    ...
    void timer_callback() {
        ...
        if (sent_ && (std::chrono::system_clock::now().time_since_epoch().count() - time_sent_) > 100000000 && time_sent_ != 0) {
            // x_goal = 200.f;
            // y_goal = -100.f;
            z_goal = 200.f;
            sent_ = false;
    ...
    void keyboard_callback(const std_msgs::msg::String::SharedPtr msg) {
        sent_ = false;
        time_sent_ = std::chrono::system_clock::now().time_since_epoch().count();
        // if (x_goal == 200.f) {
        //     x_goal = 400.f;
        // } else {
        //     x_goal = 200.f;
